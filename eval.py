...
for batch in batches:
  for duration in durations:
    for pattern in patterns:
      logfiles_filtered = [logfile for logfile in logfiles if (get_pattern(logfile) == pattern or get_pattern(logfile) == '') and get_batch(logfile) == batch and get_duration(logfile) == duration]
      if not logfiles_filtered:
        print(f'No logfiles for pattern "{pattern}", batch "{batch}" and duration "{duration}". Skipping.')
        continue
      ...
      for logfile in logfiles_filtered:
        max_threads = max(max_threads, get_threads(logfile))
        program = get_program(logfile)
      ...
      plt.title(f'Pattern: {pattern} | Batch: {batch} | Duration: {duration} s')
      plt.hlines(stats_seq.throughput, 1, max_threads, colors=colors[0], label=program_seq)
      for i, program in enumerate(programs, 1):
        stats = []
        for logfile in logfiles_filtered:
          if get_program(logfile) != program:
            continue
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.throughput for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Throughput [succ. ops / s]')
      plt.xscale('log')
      plt.yscale('log')
      ...
      if show:
        ...
        plt.savefig(f'{dir_plots}//throughput_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()

      # plot throughput (all ops)
      plt.title(f'Pattern: {pattern} | Batch: {batch} | Duration: {duration} s')
      plt.hlines(stats_seq.throughput_all, 1, max_threads, colors=colors[0], label=program_seq)
      for i, program in enumerate(programs, 1):
        stats = []
        for logfile in logfiles_filtered:
          if get_program(logfile) != program:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.throughput_all for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Throughput [succ. ops / s]')
      plt.xscale('log')
      ...
      if show:
        ...
        plt.savefig(f'{dir_plots}//throughput_all_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()
      ...
      for i, program in enumerate(programs, 1):
        ...
        for logfile in logfiles_filtered:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.throughput / stats_seq.throughput for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Speedup')
      plt.xscale('log')
      ...
      if show:
        ...
        plt.savefig(f'{dir_plots}//speedup_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()
      ...
      for i, program in enumerate(programs, 1):
        ...
        for logfile in logfiles_filtered:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.throughput_all / stats_seq.throughput_all for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Speedup')
      plt.xscale('log')
      ...
      for i, program in enumerate(programs, 1):
        ...
        for logfile in logfiles_filtered:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.deq_fail for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Dequeue fails')
      plt.xscale('log')
      plt.yscale('log')
      plt.xlim((1, max_threads))
      plt.tight_layout()
      if show:
        ...
        plt.savefig(f'{dir_plots}//deq_fails_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()
      ...
      for i, program in enumerate(programs, 1):
        ...
        for logfile in logfiles_filtered:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.freelist_insert for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Freelist inserts')
      plt.xscale('log')
      plt.yscale('log')
      plt.xlim((1, max_threads))
      plt.tight_layout()
      if show:
        ...
        plt.savefig(f'{dir_plots}//freelist_insert_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()
      ...
      for i, program in enumerate(programs, 1):
        ...
        for logfile in logfiles_filtered:
          ...
          stats.append(Stats.file(logfile))
        stats.sort()
        plt.plot([s.threads for s in stats], [s.freelist_max for s in stats], color=colors[i], marker='x', label=program)
      plt.legend()
      plt.xlabel('Threads')
      plt.ylabel('Freelist max length')
      plt.xscale('log')
      plt.yscale('log')
      plt.xlim((1, max_threads))
      plt.tight_layout()
      if show:
        ...
        plt.savefig(f'{dir_plots}//freelist_max_{pattern}_t{duration}_b{batch}.pdf')
        plt.cla()
        plt.clf()

    # plot cas success rate
    logfiles_filtered = [logfile for logfile in logfiles if get_batch(logfile) == batch and get_duration(logfile) == duration and get_program(logfile) == program_cas]
    if not logfiles_filtered:
        print(f'No "{program_cas}" logfiles batch "{batch}" and duration "{duration}". Skipping.')
        continue
    plt.title(f'Batch: {batch} | Duration: {duration} s')
    patterns = []
    for logfile in logfiles:
      pattern = get_pattern(logfile)
      if pattern and pattern not in patterns:
    ...
    for i, pattern in enumerate(patterns):
      stats = []
      for logfile in logfiles_filtered:
        if get_pattern(logfile) != pattern:
          continue
        stats.append(Stats.file(logfile))
      stats.sort()
      max_threads = max(max_threads, stats[-1].threads)
      plt.plot([s.threads for s in stats], [s.cas_succ_rate for s in stats], color=colors[i], marker='x', label=pattern)
    plt.legend()
    plt.xlabel('Threads')
    plt.ylabel('CAS success rate')
    plt.xscale('log')
    plt.yscale('log')
    plt.xlim((1, max_threads))
    plt.tight_layout()
